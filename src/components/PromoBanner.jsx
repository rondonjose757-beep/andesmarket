...
export default function PromoBanner({ product, onSelect }) {
  if (!product) {
    return (
      <div className="relative flex h-[256px] flex-col justify-center overflow-hidden rounded-3xl bg-gradient-to-br from-brand-dark to-brand px-6 py-7 text-white shadow-lg shadow-brand/20 sm:h-[300px] sm:px-8 sm:py-9">
        <p className="relative text-xs font-bold uppercase tracking-[0.15em] text-white/70">Bienvenido</p>
        <h2 className="relative mt-1.5 line-clamp-2 max-w-xs text-2xl font-black leading-[1.05] sm:text-3xl">
          Tu minimarket, a un pedido.
        </h2>
        <p className="relative mt-2 line-clamp-2 max-w-xs text-sm text-white/80">Arma tu pedido y elige retiro en tienda o delivery.</p>
      </div>
    )
  }
  ...
  return (
    <button
      ...
      className="group relative flex h-[256px] w-full flex-col justify-center overflow-hidden rounded-3xl bg-gradient-to-br from-accent to-brand px-6 py-7 text-left text-white shadow-lg shadow-ink/10 transition-transform duration-200 active:scale-[0.99] sm:h-[300px] sm:px-8 sm:py-9"
    >
      ...
      {product.image_url && (
        <div className="absolute -right-4 top-1/2 h-32 w-32 -translate-y-1/2 overflow-hidden rounded-full border-4 border-white/40 bg-white shadow-xl transition-transform duration-300 group-hover:scale-105 sm:h-40 sm:w-40">
      ...
      <h2 className="relative mt-1.5 line-clamp-2 max-w-[65%] text-2xl font-black leading-[1.05] sm:max-w-xs sm:text-3xl">{product.name}</h2>
