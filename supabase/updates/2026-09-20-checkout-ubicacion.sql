-- Permite crear pedidos delivery con dirección escrita o ubicación de Google Maps.
--
-- Aplicar después de 2026-09-19-mvp-pedidos-atomicos.sql. Reemplaza la RPC
-- existente sin cambiar su firma, permisos, cálculos ni controles de seguridad.
...
create or replace function public.create_delivery_order(payload jsonb)
returns table (
  id uuid,
  order_number bigint,
  subtotal numeric,
...
begin
  ...
  if pg_catalog.jsonb_typeof(payload -> 'customer_id') is distinct from 'string'
     ...
     or (
       payload ? 'address'
       and payload -> 'address' <> 'null'::jsonb
       and pg_catalog.jsonb_typeof(payload -> 'address') is distinct from 'string'
     )
     or (
  ...
  if v_address is null and v_google_maps_url is null then
    raise exception 'Debes indicar una dirección o compartir una ubicación de Google Maps.'
      using errcode = '22023';
  end if;

  if v_address is not null and pg_catalog.length(v_address) > 500 then
    raise exception 'La dirección no puede superar 500 caracteres.'
      using errcode = '22023';
  end if;
...
comment on function public.create_delivery_order(jsonb) is
  'Crea atómicamente un pedido delivery para el customer de auth.uid(); exige dirección o Google Maps y calcula importes desde datos confiables.';
